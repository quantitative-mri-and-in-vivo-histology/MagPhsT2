...
def cal_T2T1AM(magn_file, phas_file, mask_file, b1_file, TR, FA, phi,
               label=None, label_B1=None, outputdir=None):
    # %% prepare input data
    ...
    x0 = [t1_start, t2_start,
          amp_scale_start]  # lsq start values  for T1, T2, and amplitude scaling
    bnds = ([1, 1, 1], [10000, 1000,
                        50000])  # lsq search bounds for T1, T2, and amplitude scaling

    max_ = np.sum(mask)
    progress = pymp.shared.array((1,), dtype='int')  # Shared progress counter

    with pymp.Parallel(num_cores) as p:
        # progress = pymp.shared.array((1,), dtype='int')

        progress_bar = tqdm.tqdm(total=max_)
        # Use pymp shared array for tracking progress safely

        # with tqdm.tqdm(total=max_) as pbar:
        for i in p.range(0, max_):
            ...
            with p.lock:  # Ensure progress is updated by one thread at a time
                progress[0] += 1
                progress_bar.n = progress[0]
                progress_bar.update()
    ...
    nib.save(nib.Nifti1Image(T1_test, nii.affine), os.path.abspath(
        os.path.join(outputpath, 'T1_' + outputbasename + '.nii.gz')))

    nib.save(nib.Nifti1Image(T2_test, nii.affine), os.path.abspath(
        os.path.join(outputpath, 'T2_' + outputbasename + '.nii.gz')))

    nib.save(nib.Nifti1Image(Am_test, nii.affine), os.path.abspath(
        os.path.join(outputpath, 'Am_' + outputbasename + '.nii.gz')))
...
def main():
    ...
    cal_T2T1AM(args.magn_file, args.phas_file, args.mask_file, args.b1_file,
               args.tr, args.fa, args.phi,
